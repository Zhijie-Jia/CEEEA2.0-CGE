...
Set
            ...
            ei(i)          env. policy-related ind.      /COL,O_G/
            nei(i)         non-env. policy-related ind.  /AGR,COLP,REFO,REFG,OMIN,LGT,CMC,BMTL,STL,MTL_P,MFT,THP,HYP,WDP,NCP,SOP,CST,TSPT,SER/
...
;
Alias (u,v), (i,j,jj), (h,k), (l,al), (ei, aei);
...
Variable
                    ...
                    Td(l)                 Direct tax
                    Tz(j)                 Indirect tax
                    gammaz(i)             Relative additional energy tax rate
                    ET(i)                 Additional energy tax
                    Tm(i)                 Import tariff
...
Equation
                    ...
                    eqgammaz(ei)          Additional energy tax rate
                    eqET(i)               Additional energy tax function
...
eqCESfossil3(j)..         PFOSSIL(j)*FOSSIL(j)  =e=  PSOLID(j)*SOLID(j)+PNOS(j)*NOS(j);
...
eqTz(i)..                                Tz(i)  =e=  tauz(i)/(1-tauz(i)-gammaz(i)*(1-tauz(i)))*pz(i)*Z(i);
eqgammaz(ei)..              CFdummy*gammaz(ei)  =e=  (sum(aei,gammaz(aei))/card(ei)+Walras3)*CFdummy;
eqET(ei)..                      CFdummy*ET(ei)  =e=  gammaz(ei)*(1-tauz(ei))/(1-tauz(ei)-gammaz(ei)*(1-tauz(ei)))*pz(ei)*Z(ei) *CFdummy;
eqTm(i)..                                Tm(i)  =e=  taum(i)*pm(i)*M(i);
eqXg(i)..                                Xg(i)  =e=  mu(i)*(sum(l,Td(l)) +sum(j, Tz(j)) +sum(j, Tm(j))  -Sg +sum(j,PLC(j)) +sum(j,ET(j)) )/pq(i);

eqXv(i)..                                Xv(i)  =e=  lambda(i)*(sum(l,Sp(l)) +Sg +epsilon*Sf)/pq(i);

eqSp(l)..                                Sp(l)  =e=  ssp(l)*sum((h,j), pf(h,j)*F(h,j)*RFF(l,h));
eqSg..                                      Sg  =e=  ssg*(sum(l,Td(l)) +sum(j, Tz(j)) +sum(j, Tm(j)) +sum(j,PLC(j)) +sum(j,ET(j)) );
...
eqE(i)$(E0(i)<>0)..                       E(i)  =e=  (theta(i)**phi(i)*xie(i)*1/(1-tauz(i)-gammaz(i)*(1-tauz(i)))*pz(i)/pe(i))**(1/(1-phi(i)))*Z(i);
eqDs(i)$(E0(i)<>0)..                      D(i)  =e=  (theta(i)**phi(i)*xid(i)*1/(1-tauz(i)-gammaz(i)*(1-tauz(i)))*pz(i)/pd(i))**(1/(1-phi(i)))*Z(i);
eqpzd1(i)$(E0(i)=0)..                     D(i)  =e=  Z(i)/(1-tauz(i)-gammaz(i)*(1-tauz(i)));
...
eqCd(j)..                                Cd(j)  =e=  sum(freni,Energy(freni,j)*CO2_factor(freni))/( sum(jj,pq(jj)*X(jj,j)) +sum(h,pf(h,j)*F(h,j)) +Tz(j) +Tm(j) +PLC(j) +ET(j) );
eqDrtCspCoeff(i,j)..                    A(i,j)  =e=  pq(i)*X(i,j)/( sum(jj,pq(jj)*X(jj,j)) +sum(h,pf(h,j)*F(h,j)) +Tz(j) +Tm(j) +PLC(j) +ET(j) );
...
eqGDPCHK..                              GDPCHK  =e=  sum((h,j), pf(h,j)*F(h,j)) +sum(j, Tz(j)+Tm(j) +PLC(j) +ET(j) )  -sum(i, pq(i)*(Xv(i)+Xg(i)+sum(l,Xp(i,l))) +pe(i)*E(i) -pm(i)*M(i) ) ;
...
* gammaz is 0 if the sectors are not primary fossil energy sectors, and it is 0 if the additional energy tax is not implemented. Simlar to ET.
gammaz.fx(i)            =  0;
ET.fx(i)                =  0;
...
$ontext
In all CF scenario, we should consider the emission target and ADEEI, as well as
the changing parameters in BAU scenario.
$offtext
...
*  if the base period is 2018, then: when ord(t)=1, the simulation period is 2019   ...    ord(t)=42, period=2060
*  if ord(t)>=3, it means the policy is introduced in and after 2021
if(          ord(t)  >= 3,
           BAUDummy  =  0;
            CFDummy  =  1;
   gammaz.lo("COL")  =  -inf;
...
if(ord(t)=        1        ,C_TFP.FX=C_TFP1("2019");        C_AEEI.FX=C_AEEI1("2019");        C_ELE.fx=C_ELE1("2019");        penebau(j)=penebau1("2019",j);        pzbau(j)=pzbau1("2019",j);        pqbau(j)=pqbau1("2019",j);        xpbau(j,l)=xpbau1("2019",j,l);        );
if(ord(t)=        2        ,C_TFP.FX=C_TFP1("2020");        C_AEEI.FX=C_AEEI1("2020");        C_ELE.fx=C_ELE1("2020");        penebau(j)=penebau1("2020",j);        pzbau(j)=pzbau1("2020",j);        pqbau(j)=pqbau1("2020",j);        xpbau(j,l)=xpbau1("2020",j,l);        );
if(ord(t)=        3        ,C_TFP.FX=C_TFP1("2021");        C_AEEI.FX=C_AEEI1("2021");        C_ELE.fx=C_ELE1("2021");        penebau(j)=penebau1("2021",j);        pzbau(j)=pzbau1("2021",j);        pqbau(j)=pqbau1("2021",j);        xpbau(j,l)=xpbau1("2021",j,l);        Emissions.fx=CFemissions("2021");        );
if(ord(t)=        4        ,C_TFP.FX=C_TFP1("2022");        C_AEEI.FX=C_AEEI1("2022");        C_ELE.fx=C_ELE1("2022");        penebau(j)=penebau1("2022",j);        pzbau(j)=pzbau1("2022",j);        pqbau(j)=pqbau1("2022",j);        xpbau(j,l)=xpbau1("2022",j,l);        Emissions.fx=CFemissions("2022");        );
if(ord(t)=        5        ,C_TFP.FX=C_TFP1("2023");        C_AEEI.FX=C_AEEI1("2023");        C_ELE.fx=C_ELE1("2023");        penebau(j)=penebau1("2023",j);        pzbau(j)=pzbau1("2023",j);        pqbau(j)=pqbau1("2023",j);        xpbau(j,l)=xpbau1("2023",j,l);        Emissions.fx=CFemissions("2023");        );
if(ord(t)=        6        ,C_TFP.FX=C_TFP1("2024");        C_AEEI.FX=C_AEEI1("2024");        C_ELE.fx=C_ELE1("2024");        penebau(j)=penebau1("2024",j);        pzbau(j)=pzbau1("2024",j);        pqbau(j)=pqbau1("2024",j);        xpbau(j,l)=xpbau1("2024",j,l);        Emissions.fx=CFemissions("2024");        );
if(ord(t)=        7        ,C_TFP.FX=C_TFP1("2025");        C_AEEI.FX=C_AEEI1("2025");        C_ELE.fx=C_ELE1("2025");        penebau(j)=penebau1("2025",j);        pzbau(j)=pzbau1("2025",j);        pqbau(j)=pqbau1("2025",j);        xpbau(j,l)=xpbau1("2025",j,l);        Emissions.fx=CFemissions("2025");        );
if(ord(t)=        8        ,C_TFP.FX=C_TFP1("2026");        C_AEEI.FX=C_AEEI1("2026");        C_ELE.fx=C_ELE1("2026");        penebau(j)=penebau1("2026",j);        pzbau(j)=pzbau1("2026",j);        pqbau(j)=pqbau1("2026",j);        xpbau(j,l)=xpbau1("2026",j,l);        Emissions.fx=CFemissions("2026");        );
if(ord(t)=        9        ,C_TFP.FX=C_TFP1("2027");        C_AEEI.FX=C_AEEI1("2027");        C_ELE.fx=C_ELE1("2027");        penebau(j)=penebau1("2027",j);        pzbau(j)=pzbau1("2027",j);        pqbau(j)=pqbau1("2027",j);        xpbau(j,l)=xpbau1("2027",j,l);        Emissions.fx=CFemissions("2027");        );
if(ord(t)=        10        ,C_TFP.FX=C_TFP1("2028");        C_AEEI.FX=C_AEEI1("2028");        C_ELE.fx=C_ELE1("2028");        penebau(j)=penebau1("2028",j);        pzbau(j)=pzbau1("2028",j);        pqbau(j)=pqbau1("2028",j);        xpbau(j,l)=xpbau1("2028",j,l);        Emissions.fx=CFemissions("2028");        );
if(ord(t)=        11        ,C_TFP.FX=C_TFP1("2029");        C_AEEI.FX=C_AEEI1("2029");        C_ELE.fx=C_ELE1("2029");        penebau(j)=penebau1("2029",j);        pzbau(j)=pzbau1("2029",j);        pqbau(j)=pqbau1("2029",j);        xpbau(j,l)=xpbau1("2029",j,l);        Emissions.fx=CFemissions("2029");        );
if(ord(t)=        12        ,C_TFP.FX=C_TFP1("2030");        C_AEEI.FX=C_AEEI1("2030");        C_ELE.fx=C_ELE1("2030");        penebau(j)=penebau1("2030",j);        pzbau(j)=pzbau1("2030",j);        pqbau(j)=pqbau1("2030",j);        xpbau(j,l)=xpbau1("2030",j,l);        Emissions.fx=CFemissions("2030");        );
if(ord(t)=        13        ,C_TFP.FX=C_TFP1("2031");        C_AEEI.FX=C_AEEI1("2031");        C_ELE.fx=C_ELE1("2031");        penebau(j)=penebau1("2031",j);        pzbau(j)=pzbau1("2031",j);        pqbau(j)=pqbau1("2031",j);        xpbau(j,l)=xpbau1("2031",j,l);        Emissions.fx=CFemissions("2031");        );
if(ord(t)=        14        ,C_TFP.FX=C_TFP1("2032");        C_AEEI.FX=C_AEEI1("2032");        C_ELE.fx=C_ELE1("2032");        penebau(j)=penebau1("2032",j);        pzbau(j)=pzbau1("2032",j);        pqbau(j)=pqbau1("2032",j);        xpbau(j,l)=xpbau1("2032",j,l);        Emissions.fx=CFemissions("2032");        );
if(ord(t)=        15        ,C_TFP.FX=C_TFP1("2033");        C_AEEI.FX=C_AEEI1("2033");        C_ELE.fx=C_ELE1("2033");        penebau(j)=penebau1("2033",j);        pzbau(j)=pzbau1("2033",j);        pqbau(j)=pqbau1("2033",j);        xpbau(j,l)=xpbau1("2033",j,l);        Emissions.fx=CFemissions("2033");        );
if(ord(t)=        16        ,C_TFP.FX=C_TFP1("2034");        C_AEEI.FX=C_AEEI1("2034");        C_ELE.fx=C_ELE1("2034");        penebau(j)=penebau1("2034",j);        pzbau(j)=pzbau1("2034",j);        pqbau(j)=pqbau1("2034",j);        xpbau(j,l)=xpbau1("2034",j,l);        Emissions.fx=CFemissions("2034");        );
if(ord(t)=        17        ,C_TFP.FX=C_TFP1("2035");        C_AEEI.FX=C_AEEI1("2035");        C_ELE.fx=C_ELE1("2035");        penebau(j)=penebau1("2035",j);        pzbau(j)=pzbau1("2035",j);        pqbau(j)=pqbau1("2035",j);        xpbau(j,l)=xpbau1("2035",j,l);        Emissions.fx=CFemissions("2035");        );
if(ord(t)=        18        ,C_TFP.FX=C_TFP1("2036");        C_AEEI.FX=C_AEEI1("2036");        C_ELE.fx=C_ELE1("2036");        penebau(j)=penebau1("2036",j);        pzbau(j)=pzbau1("2036",j);        pqbau(j)=pqbau1("2036",j);        xpbau(j,l)=xpbau1("2036",j,l);        Emissions.fx=CFemissions("2036");        );
if(ord(t)=        19        ,C_TFP.FX=C_TFP1("2037");        C_AEEI.FX=C_AEEI1("2037");        C_ELE.fx=C_ELE1("2037");        penebau(j)=penebau1("2037",j);        pzbau(j)=pzbau1("2037",j);        pqbau(j)=pqbau1("2037",j);        xpbau(j,l)=xpbau1("2037",j,l);        Emissions.fx=CFemissions("2037");        );
if(ord(t)=        20        ,C_TFP.FX=C_TFP1("2038");        C_AEEI.FX=C_AEEI1("2038");        C_ELE.fx=C_ELE1("2038");        penebau(j)=penebau1("2038",j);        pzbau(j)=pzbau1("2038",j);        pqbau(j)=pqbau1("2038",j);        xpbau(j,l)=xpbau1("2038",j,l);        Emissions.fx=CFemissions("2038");        );
if(ord(t)=        21        ,C_TFP.FX=C_TFP1("2039");        C_AEEI.FX=C_AEEI1("2039");        C_ELE.fx=C_ELE1("2039");        penebau(j)=penebau1("2039",j);        pzbau(j)=pzbau1("2039",j);        pqbau(j)=pqbau1("2039",j);        xpbau(j,l)=xpbau1("2039",j,l);        Emissions.fx=CFemissions("2039");        );
if(ord(t)=        22        ,C_TFP.FX=C_TFP1("2040");        C_AEEI.FX=C_AEEI1("2040");        C_ELE.fx=C_ELE1("2040");        penebau(j)=penebau1("2040",j);        pzbau(j)=pzbau1("2040",j);        pqbau(j)=pqbau1("2040",j);        xpbau(j,l)=xpbau1("2040",j,l);        Emissions.fx=CFemissions("2040");        );
if(ord(t)=        23        ,C_TFP.FX=C_TFP1("2041");        C_AEEI.FX=C_AEEI1("2041");        C_ELE.fx=C_ELE1("2041");        penebau(j)=penebau1("2041",j);        pzbau(j)=pzbau1("2041",j);        pqbau(j)=pqbau1("2041",j);        xpbau(j,l)=xpbau1("2041",j,l);        Emissions.fx=CFemissions("2041");        );
if(ord(t)=        24        ,C_TFP.FX=C_TFP1("2042");        C_AEEI.FX=C_AEEI1("2042");        C_ELE.fx=C_ELE1("2042");        penebau(j)=penebau1("2042",j);        pzbau(j)=pzbau1("2042",j);        pqbau(j)=pqbau1("2042",j);        xpbau(j,l)=xpbau1("2042",j,l);        Emissions.fx=CFemissions("2042");        );
if(ord(t)=        25        ,C_TFP.FX=C_TFP1("2043");        C_AEEI.FX=C_AEEI1("2043");        C_ELE.fx=C_ELE1("2043");        penebau(j)=penebau1("2043",j);        pzbau(j)=pzbau1("2043",j);        pqbau(j)=pqbau1("2043",j);        xpbau(j,l)=xpbau1("2043",j,l);        Emissions.fx=CFemissions("2043");        );
if(ord(t)=        26        ,C_TFP.FX=C_TFP1("2044");        C_AEEI.FX=C_AEEI1("2044");        C_ELE.fx=C_ELE1("2044");        penebau(j)=penebau1("2044",j);        pzbau(j)=pzbau1("2044",j);        pqbau(j)=pqbau1("2044",j);        xpbau(j,l)=xpbau1("2044",j,l);        Emissions.fx=CFemissions("2044");        );
if(ord(t)=        27        ,C_TFP.FX=C_TFP1("2045");        C_AEEI.FX=C_AEEI1("2045");        C_ELE.fx=C_ELE1("2045");        penebau(j)=penebau1("2045",j);        pzbau(j)=pzbau1("2045",j);        pqbau(j)=pqbau1("2045",j);        xpbau(j,l)=xpbau1("2045",j,l);        Emissions.fx=CFemissions("2045");        );
if(ord(t)=        28        ,C_TFP.FX=C_TFP1("2046");        C_AEEI.FX=C_AEEI1("2046");        C_ELE.fx=C_ELE1("2046");        penebau(j)=penebau1("2046",j);        pzbau(j)=pzbau1("2046",j);        pqbau(j)=pqbau1("2046",j);        xpbau(j,l)=xpbau1("2046",j,l);        Emissions.fx=CFemissions("2046");        );
if(ord(t)=        29        ,C_TFP.FX=C_TFP1("2047");        C_AEEI.FX=C_AEEI1("2047");        C_ELE.fx=C_ELE1("2047");        penebau(j)=penebau1("2047",j);        pzbau(j)=pzbau1("2047",j);        pqbau(j)=pqbau1("2047",j);        xpbau(j,l)=xpbau1("2047",j,l);        Emissions.fx=CFemissions("2047");        );
if(ord(t)=        30        ,C_TFP.FX=C_TFP1("2048");        C_AEEI.FX=C_AEEI1("2048");        C_ELE.fx=C_ELE1("2048");        penebau(j)=penebau1("2048",j);        pzbau(j)=pzbau1("2048",j);        pqbau(j)=pqbau1("2048",j);        xpbau(j,l)=xpbau1("2048",j,l);        Emissions.fx=CFemissions("2048");        );
if(ord(t)=        31        ,C_TFP.FX=C_TFP1("2049");        C_AEEI.FX=C_AEEI1("2049");        C_ELE.fx=C_ELE1("2049");        penebau(j)=penebau1("2049",j);        pzbau(j)=pzbau1("2049",j);        pqbau(j)=pqbau1("2049",j);        xpbau(j,l)=xpbau1("2049",j,l);        Emissions.fx=CFemissions("2049");        );
if(ord(t)=        32        ,C_TFP.FX=C_TFP1("2050");        C_AEEI.FX=C_AEEI1("2050");        C_ELE.fx=C_ELE1("2050");        penebau(j)=penebau1("2050",j);        pzbau(j)=pzbau1("2050",j);        pqbau(j)=pqbau1("2050",j);        xpbau(j,l)=xpbau1("2050",j,l);        Emissions.fx=CFemissions("2050");        );
if(ord(t)=        33        ,C_TFP.FX=C_TFP1("2051");        C_AEEI.FX=C_AEEI1("2051");        C_ELE.fx=C_ELE1("2051");        penebau(j)=penebau1("2051",j);        pzbau(j)=pzbau1("2051",j);        pqbau(j)=pqbau1("2051",j);        xpbau(j,l)=xpbau1("2051",j,l);        Emissions.fx=CFemissions("2051");        );
if(ord(t)=        34        ,C_TFP.FX=C_TFP1("2052");        C_AEEI.FX=C_AEEI1("2052");        C_ELE.fx=C_ELE1("2052");        penebau(j)=penebau1("2052",j);        pzbau(j)=pzbau1("2052",j);        pqbau(j)=pqbau1("2052",j);        xpbau(j,l)=xpbau1("2052",j,l);        Emissions.fx=CFemissions("2052");        );
if(ord(t)=        35        ,C_TFP.FX=C_TFP1("2053");        C_AEEI.FX=C_AEEI1("2053");        C_ELE.fx=C_ELE1("2053");        penebau(j)=penebau1("2053",j);        pzbau(j)=pzbau1("2053",j);        pqbau(j)=pqbau1("2053",j);        xpbau(j,l)=xpbau1("2053",j,l);        Emissions.fx=CFemissions("2053");        );
if(ord(t)=        36        ,C_TFP.FX=C_TFP1("2054");        C_AEEI.FX=C_AEEI1("2054");        C_ELE.fx=C_ELE1("2054");        penebau(j)=penebau1("2054",j);        pzbau(j)=pzbau1("2054",j);        pqbau(j)=pqbau1("2054",j);        xpbau(j,l)=xpbau1("2054",j,l);        Emissions.fx=CFemissions("2054");        );
if(ord(t)=        37        ,C_TFP.FX=C_TFP1("2055");        C_AEEI.FX=C_AEEI1("2055");        C_ELE.fx=C_ELE1("2055");        penebau(j)=penebau1("2055",j);        pzbau(j)=pzbau1("2055",j);        pqbau(j)=pqbau1("2055",j);        xpbau(j,l)=xpbau1("2055",j,l);        Emissions.fx=CFemissions("2055");        );
if(ord(t)=        38        ,C_TFP.FX=C_TFP1("2056");        C_AEEI.FX=C_AEEI1("2056");        C_ELE.fx=C_ELE1("2056");        penebau(j)=penebau1("2056",j);        pzbau(j)=pzbau1("2056",j);        pqbau(j)=pqbau1("2056",j);        xpbau(j,l)=xpbau1("2056",j,l);        Emissions.fx=CFemissions("2056");        );
if(ord(t)=        39        ,C_TFP.FX=C_TFP1("2057");        C_AEEI.FX=C_AEEI1("2057");        C_ELE.fx=C_ELE1("2057");        penebau(j)=penebau1("2057",j);        pzbau(j)=pzbau1("2057",j);        pqbau(j)=pqbau1("2057",j);        xpbau(j,l)=xpbau1("2057",j,l);        Emissions.fx=CFemissions("2057");        );
if(ord(t)=        40        ,C_TFP.FX=C_TFP1("2058");        C_AEEI.FX=C_AEEI1("2058");        C_ELE.fx=C_ELE1("2058");        penebau(j)=penebau1("2058",j);        pzbau(j)=pzbau1("2058",j);        pqbau(j)=pqbau1("2058",j);        xpbau(j,l)=xpbau1("2058",j,l);        Emissions.fx=CFemissions("2058");        );
if(ord(t)=        41        ,C_TFP.FX=C_TFP1("2059");        C_AEEI.FX=C_AEEI1("2059");        C_ELE.fx=C_ELE1("2059");        penebau(j)=penebau1("2059",j);        pzbau(j)=pzbau1("2059",j);        pqbau(j)=pqbau1("2059",j);        xpbau(j,l)=xpbau1("2059",j,l);        Emissions.fx=CFemissions("2059");        );
if(ord(t)=        42        ,C_TFP.FX=C_TFP1("2060");        C_AEEI.FX=C_AEEI1("2060");        C_ELE.fx=C_ELE1("2060");        penebau(j)=penebau1("2060",j);        pzbau(j)=pzbau1("2060",j);        pqbau(j)=pqbau1("2060",j);        xpbau(j,l)=xpbau1("2060",j,l);        Emissions.fx=CFemissions("2060");        );
